...
Tunnel-aligned splash routing (recommended):
...
3. `[preflight] splash check: ok (HEAD 200)`
4. `Worker splash source: mode=ascii, image_url=https://<active-tunnel>/assets/email-splash.png, data_uri_fallback=no`

If preflight warnings are present, startup continues by default. Set `COMMENT_WORKER_PREFLIGHT_STRICT=true` to fail fast when tunnel/splash checks are unhealthy.
