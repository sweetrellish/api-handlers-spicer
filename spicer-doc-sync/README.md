...
### Worker Tunnel/Splash Preflight

`tagger/run_comment_worker.sh` now runs a startup preflight (`tagger/preflight_comment_worker.sh`) before launching `comment_worker.py`.

What it checks:

1. `WEBHOOK_URL` is set and valid.
2. Splash URL selection is clear (`COMMENT_WORKER_SPLASH_IMAGE_URL=auto` derives from `WEBHOOK_URL` host).
3. Splash image endpoint is reachable (`/assets/email-splash.png`).
4. Warns if splash host and webhook host diverge (common tunnel-rotation drift).

The email template also keeps the `TAG ALERT` badge on one line so Gmail mobile does not reflow it awkwardly.


Recommended settings in `tagger/.env`:

```dotenv
WEBHOOK_URL=https://<active-tunnel>.trycloudflare.com/webhook/companycam
COMMENT_WORKER_SPLASH_IMAGE_URL=auto
```

Optional preflight controls:

```dotenv
COMMENT_WORKER_PREFLIGHT_ENABLED=true
COMMENT_WORKER_PREFLIGHT_STRICT=false
COMMENT_WORKER_PREFLIGHT_TIMEOUT_SECONDS=6
```

`STRICT=true` makes startup fail fast if preflight warnings are detected.
