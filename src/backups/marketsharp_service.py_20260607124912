...
class MarketSharpService:
    def __init__(self):
        ...
        self._odata_entity_error_log_window_seconds = max(
            30,
            int(os.getenv('MARKETSHARP_ODATA_ENTITY_LOG_WINDOW_SECONDS', '300')),
        )
        self._odata_entity_error_log_state = {}
        self.rest_headers = {
    ...
    def _throttled_entity_log(self, uri, exc):
        # Collapse GUID-specific entity URLs into a stable bucket to reduce log noise.
        bucket = re.sub(r"\('[0-9A-Fa-f-]{36}'\)", "('<id>')", uri)
        bucket = re.sub(r"\([0-9A-Fa-f-]{36}\)", "(<id>)", bucket)
        if '?' in bucket:
            bucket = bucket.split('?', 1)[0]

        now = time.time()
        state = self._odata_entity_error_log_state.get(bucket)
        window = self._odata_entity_error_log_window_seconds
        ...
        is_expected_400 = (
            ...
            and getattr(getattr(exc, 'response', None), 'status_code', None) == 400
            and (
                "/Notes('" in uri and "')/Contact" in uri
                or "/Contacts('" in uri and "')/Address" in uri
            )
        )
        is_expected_timeout = isinstance(exc, requests.Timeout)
        level = logging.INFO if (is_expected_400 or is_expected_timeout) else logging.WARNING

        if state and state['suppressed'] > 0:
            logging.log(
                level,
                'Suppressed %s repeated OData entity fetch failures for %s over %ss',
                state['suppressed'],
                bucket,
                int(now - state['window_start']),
            )

        if is_expected_400:
            msg = 'OData entity relation missing/unavailable %s: %s'
        elif is_expected_timeout:
            msg = 'OData entity lookup timed out %s: %s'
        else:
            msg = 'Failed OData entity fetch %s: %s'
        logging.log(level, msg, uri, exc)
        self._odata_entity_error_log_state[bucket] = {
            'window_start': now,
            'suppressed': 0,
        }
    ...
    def _fetch_odata_entity(self, uri):
        ...
        try:
            ...
            self._throttled_entity_log(normalized_uri, exc)
    ...
    def create_activity(self, payload, dry_run=False):
        """Create a MarketSharp activity through the OData Activities entity."""
        if dry_run:
            return {'dry_run': True, 'payload': payload}

        if self.effective_mode != 'odata_write':
            logging.info('Skipping activity create: MarketSharp mode is %s', self.effective_mode)
            return None

        try:
            response = requests.post(
                f'{self.odata_url}/Activities',
                headers=self._odata_headers(),
                json=payload,
                timeout=self.timeout,
            )
            ...
            verbose_payload = dict(payload)
            verbose_payload['__metadata'] = {'type': 'MSharpModel.Activity'}
            response2 = requests.post(
                f'{self.odata_url}/Activities',
                headers=self._odata_headers(verbose=True),
                json=verbose_payload,
            ...
            logging.error(
                'OData activity create failed contact=%s status1=%s body1=%s status2=%s body2=%s',
            ...
            logging.exception(
                'Error posting OData activity for contact %s: %s',
                payload.get('contactId'),
                str(e),
            )
            return None
